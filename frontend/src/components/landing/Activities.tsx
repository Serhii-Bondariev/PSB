...
const Activities: React.FC = () => {
  return <section id="activities" className="py-16 bg-light">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <h2 className="text-3xl font-bold text-dark mb-2">Наші секції</h2>
          <div className="w-20 h-1 bg-accent mx-auto mb-4"></div>
          <p className="text-lg text-gray-600 max-w-3xl mx-auto">
            Безкоштовні спортивні секції для дітей. Приєднуйтесь до нас та
            розвивайте свої спортивні навички під керівництвом професійних
            тренерів.
          </p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {activities.map(activity => <div key={activity.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
              <div className="h-64 overflow-hidden">
                <img src={activity.image} alt={activity.name} className="w-full h-full object-cover transition-transform hover:scale-105" />
              </div>
              <div className="p-6">
                ...
                <Link to={`/section/${activity.id}`} className="bg-secondary text-white px-6 py-3 rounded-full hover:bg-primary transition-colors inline-block">
        ...
        <div className="text-center mt-12 bg-white p-8 rounded-lg shadow-md">
          ...
          <p className="text-gray-600 mb-6">
            Всі заняття є безкоштовними. Для запису необхідно лише медична
            довідка про відсутність протипоказань до занять спортом.
          </p>
          <Link to="/contacts" className="bg-primary text-white px-8 py-3 rounded-full font-bold hover:bg-dark transition-colors inline-flex items-center">
